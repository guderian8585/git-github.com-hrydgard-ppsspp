...
int Atrac2::SetData(u32 buffer, u32 readSize, u32 bufferSize, int outputChannels, int successCode) {
	...
	if (bufferSize >= track_.fileSize) {
		...
		if (track_.loopEndSample <= 0) {
			...
			WARN_LOG(ME, "Streaming detected - no loop");
			...
			WARN_LOG(ME, "Streaming detected - loop from end");
		} else {
			bufferState_ = ATRAC_STATUS_STREAMED_LOOP_WITH_TRAILER;
			WARN_LOG(ME, "Streaming detected - loop with trailer");
		}
	}
...
u32 Atrac2::DecodeData(u8 *outbuf, u32 outbufPtr, u32 *SamplesNum, u32 *finish, int *remains) {
	...
	*SamplesNum = samplesWritten;
	...
	if (LoopNum() != 0 && currentSample_ >= track_.loopEndSample) {
		...
		DEBUG_LOG(ME, "sceAtrac: Hit loop (%d) at %d (currentSample_ == %d), seeking to sample %d", loopNum_, track_.loopEndSample, currentSample_, track_.loopStartSample);
	...
	if (bufPos_ >= bufSize_) {
		DEBUG_LOG(ME, "sceAtrac: Hit end of input buffer (%d >= %d)! LoopNum=%d. Setting finish=1. currentSample: %d loopEndSample: %d", bufPos_, bufSize_, loopNum_, currentSample_, track_.loopEndSample);
		*finish = 1;
	} else {
		*finish = 0;
	}
